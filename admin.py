...
def _paths(name):
    ...
    if hasattr(socket, "AF_UNIX"):
        endpoint = f"/tmp/bu-{n}.sock"
    else:
        port = int(os.environ.get("BU_TCP_PORT", 31000 + (zlib.crc32(n.encode()) % 10000)))
...
def ensure_daemon(wait=60.0, name=None, env=None):
    ...
    for attempt in (0, 1):
        ...
        if local and attempt == 0 and _needs_chrome_remote_debugging_prompt(msg):
            ...
            restart_daemon(name)
            continue
        raise RuntimeError(msg or f"daemon {name or NAME} didn't come up -- check {TMP / f'bu-{name or NAME}.log'}")
...
def restart_daemon(name=None):
    ...
    try:
        family = socket.AF_UNIX if isinstance(sock, str) else socket.AF_INET
        s = socket.socket(family, socket.SOCK_STREAM)
        s.settimeout(5)
        s.connect(sock)
