...
import base64, json, os, socket, time, urllib.request, zlib
...
def _socket_addr():
    if hasattr(socket, "AF_UNIX"):
        return socket.AF_UNIX, SOCK
    port = int(os.environ.get("BU_TCP_PORT", 31000 + (zlib.crc32(NAME.encode()) % 10000)))
    return socket.AF_INET, ("127.0.0.1", port)


def _send(req):
    family, addr = _socket_addr()
...
def page_info():
    ...
    r = cdp("Runtime.evaluate",
            expression="const t=document.title.startsWith('\U0001F7E2 ')?document.title.slice(3):document.title;JSON.stringify({url:location.href,title:t,w:innerWidth,h:innerHeight,sx:scrollX,sy:scrollY,pw:document.documentElement.scrollWidth,ph:document.documentElement.scrollHeight})",
            returnByValue=True)
...
def _mark_tab():
    """Prepend [BH] to tab title so the user can see which tab the agent controls."""
    try: cdp("Runtime.evaluate", expression="if(document.title.startsWith('\U0001F7E2 '))document.title=document.title.slice(3);if(!document.title.startsWith('[BH] '))document.title='[BH] '+document.title")
...
def switch_tab(target):
    ...
    # Unmark old tab
    try: cdp("Runtime.evaluate", expression="if(document.title.startsWith('[BH] '))document.title=document.title.slice(5);if(document.title.startsWith('\U0001F7E2 '))document.title=document.title.slice(3)")
    except Exception: pass
