...
import asyncio, json, os, socket, sys, tempfile, time, urllib.request, zlib
...
class Daemon:
    ...
    async def start(self):
        ...
        mark_js = "if(document.title.startsWith('\U0001F7E2 '))document.title=document.title.slice(3);if(!document.title.startsWith('[BH] '))document.title='[BH] '+document.title"
    ...
    async def handle(self, req):
        ...
        if meta == "set_session":
            ...
            try:
                ...
                await asyncio.wait_for(self.cdp.send_raw("Runtime.evaluate", {"expression": "if(document.title.startsWith('\U0001F7E2 '))document.title=document.title.slice(3);if(!document.title.startsWith('[BH] '))document.title='[BH] '+document.title"}, session_id=self.session), timeout=2)
...
async def serve(d):
    ...
    if hasattr(socket, "AF_UNIX") and family == socket.AF_UNIX and os.path.exists(SOCK):
        os.unlink(SOCK)
    ...
    if hasattr(socket, "AF_UNIX") and family == socket.AF_UNIX:
        ...
        server = await asyncio.start_server(handler, host=addr[0], port=addr[1])
...
def already_running():
    try:
        ...
        s.connect(addr); s.close(); return True
