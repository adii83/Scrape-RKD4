...
PROGRESS_FILE = OUT_DIR / "alumni_facebook_progress.json"
...
def wait_until_unblocked(context):
    while True:
        ...
        print(f"[FB] challenge/rate-limit terdeteksi ({context}), tunggu {int(wait_sec)} detik lalu cek ulang...")
        wait(wait_sec)
...
def clean_position_text(value):
    ...
    if re.search(r"^(follow|following|message|add friend|view profile)$", text, flags=re.IGNORECASE):
        return ""
    return text
...
def make_output_paths(batch_start, batch_end):
    base = f"alumni_fb_{batch_start}_{batch_end}"
    return {
...
def merge_base(linkedin_row, instagram_row):
    merged = {}
    merged.update(linkedin_row or {})
    for k, v in (instagram_row or {}).items():
        if not clean_text(str(merged.get(k, ""))) and clean_text(str(v)):
            merged[k] = v
    return merged


def search_facebook_candidates(query, engine_name, engine_template):
...
(() => {
  ...
  for (const href of links) {
    ...
    if (/facebook\.com\/(share|watch|reel|photo|events|groups|marketplace|help)\b/i.test(clean)) continue;
    const profileLike = /facebook\.com\/(profile\.php|[A-Za-z0-9.]{3,})/i.test(clean);
    if (!profileLike) continue;
    if (seen.has(clean)) continue;
    seen.add(clean);
    out.push({facebook_url: clean});
...
def pick_valid_facebook_candidate(candidates):
    for item in candidates or []:
        if not is_blocked_facebook_url(item.get("facebook_url", "")):
            return item
    return None


def read_facebook_profile(facebook_url):
    safe_navigate(facebook_url, "open facebook profile", PROFILE_DELAY_MIN, PROFILE_DELAY_MAX)
    return js(
        r"""
(() => {
  ...
  const links = Array.from(document.querySelectorAll('a[href]')).map(a => a.href);
  const extLinks = links.filter(h => !h.includes('facebook.com')).slice(0, 10);
...
def is_blocked_facebook_url(url):
    u = clean_text(url).lower().rstrip("/")
    if not u:
        return False
    return any(k in u for k in BLOCKED_FACEBOOK_URL_KEYWORDS)


def merge_with_facebook(base_row, fb_info):
    row = dict(base_row)
    body = clean_text(fb_info.get("body_text", ""))
    title = clean_text(fb_info.get("title", ""))
    heading = clean_position_text(fb_info.get("heading", ""))
    combined = clean_text(body + " " + title + " " + heading)
    company = extract_work_from_text(combined)
    email = first_email(combined)
    phone = first_phone(combined)
    ext_links = fb_info.get("external_links", []) or []
    work_social = ""
    ...
    row["alamat_sosial_media_facebook"] = pick_first_non_empty(row.get("alamat_sosial_media_facebook", ""), fb_info.get("facebook_url", ""))
    if is_blocked_facebook_url(row.get("alamat_sosial_media_facebook", "")):
        row["alamat_sosial_media_facebook"] = ""
    ...
    row["posisi"] = pick_first_non_empty(row.get("posisi", ""), heading)
...
def run_batch(batch_start, batch_end, rows_done_before, total_target, all_rows, columns, linkedin_map, instagram_map):
    ...
    for idx, alumni in enumerate(alumni_rows):
        ...
        base = merge_base(linkedin_map.get(row_excel, {}), instagram_map.get(row_excel, {}))
        row = {
            "row_excel": row_excel,
            "nama_lulusan": name,
        ...
        queries = [
            f"{name} {DEFAULT_UNIVERSITY} facebook",
            f"{name} facebook",
        ]
        candidates = []
        top = None
        ...
        for q in queries:
            for engine_name, engine_template in SEARCH_ENGINES:
                print(f"row={row_excel} [FB] coba query: {q} via {engine_name}")
                candidates = search_facebook_candidates(q, engine_name, engine_template)
                top = pick_valid_facebook_candidate(candidates)
                if top:
                    used_query = q
        ...
        if not used_query:
            ...
            top = None
            used_engine = ""
        row["query_dipakai_facebook"] = used_query
        row["search_engine_facebook"] = used_engine

        if top:
            info = read_facebook_profile(top["facebook_url"])
            info["facebook_url"] = top["facebook_url"]
            row = merge_with_facebook(row, info)
        ...
        all_rows.append(row)
        print(f"[{global_idx}/{total_target}] row={row_excel} {name}: {row.get('alamat_sosial_media_facebook', '') or 'tidak ada kandidat'}")
...
def run_lookup():
    ...
    columns = [
        ...
        "query_dipakai",
        "mode_pencarian",
        "query_dipakai_facebook",
        "search_engine_facebook",
    ...
    while batch_start <= END_ROW:
        ...
        batch_rows = run_batch(batch_start, batch_end, rows_done_before, total_target, all_rows, columns, linkedin_map, instagram_map)
        rows_done_before += len(batch_rows)

        all_json = OUT_DIR / f"alumni_fb_all_{START_ROW}_{END_ROW}.json"
        all_csv = OUT_DIR / f"alumni_fb_all_{START_ROW}_{END_ROW}.csv"
        all_xlsx = OUT_DIR / f"alumni_fb_all_{START_ROW}_{END_ROW}.xlsx"
